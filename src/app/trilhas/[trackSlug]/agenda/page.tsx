// ===========================================================================
// Amplify Hub · /trilhas/[trackSlug]/agenda — calendário da trilha
// ===========================================================================
// Timeline vertical das mentorias agendadas para a trilha do aluno (Lote 2
// §2.3). Próximas em ordem ascendente, agrupadas por mês para que semanas
// densas se leiam rápido. Últimas 5 passadas exibidas em bloco discreto
// abaixo, com link para a gravação quando houver `recording_url`.
//
// RLS já filtra: aluno só vê lives de trilhas onde tem entitlement; admin
// (claim app_metadata.admin) vê tudo via 0003_admin_rls_bypass.
// ===========================================================================
...
export async function generateMetadata({
  ...
  return {
    title: track
      ? `Agenda · ${TRACK_LABELS[track]} · Amplify Hub`
      : 'Agenda · Amplify Hub',
  }
}
...
function monthKey(iso: string) {
  ...
  // Agrupa pela TZ de SP — dois eventos no mesmo "abril" para o admin não
  // devem se separar por causa de UTC vs SP.
...
export default async function AgendaPage({
  ...
  // Filtramos a track no client-side da query via inner join: a RLS já
  // bloqueia tracks sem entitlement, mas o filtro explícito limita o payload
  // ao escopo da página de agenda solicitada (admin tem acesso a Atlas E
  // Amplify, então sem o filtro veria as duas misturadas aqui).
  const [upcomingRes, pastRes] = await Promise.all([
    supabase
      .from('live_sessions')
      .select(
        'id, title, scheduled_for, duration_minutes, is_active, recording_url, lessons!inner(title, modules!inner(title, track))',
      )
      .eq('lessons.modules.track', track)
  ...
  const upcomingRaw = (upcomingRes.data ?? []) as unknown as AgendaSession[]
  const pastRaw = (pastRes.data ?? []) as unknown as AgendaSession[]
  ...
  const upcoming = normalize(upcomingRaw)
  const past = normalize(pastRaw)

  // Agrupa por mês mantendo a ordem original (asc para upcoming).
  const grouped: Array<{ key: string; label: string; items: typeof upcoming }> =
    []
  ...
  return (
    <main className="mx-auto max-w-4xl px-5 py-10 md:px-8 md:py-14">
      ...
      >
        ...
        <ChevronRight className="size-3.5 opacity-60" aria-hidden />
        <span className="text-(--color-text-secondary)">Agenda</span>
      </nav>
      ...
      <header className="mb-10 space-y-3">
        ...
        <h1 className="font-serif text-3xl font-semibold leading-snug tracking-tight md:text-4xl">
          Calendário de mentorias · {trackLabel}
        </h1>
        <p className="font-sans text-sm text-(--color-text-secondary) md:text-base">
          Todas as lives marcadas para esta trilha. A sala abre 15 minutos
          antes do horário.
        </p>
      </header>

      {/* === Próximas mentorias (timeline agrupada por mês) === */}
      <section aria-labelledby="upcoming-heading" className="mb-12 space-y-6">
        ...
        {grouped.length === 0 ? (
          ...
          >
            ...
            Nenhuma mentoria agendada para esta trilha no momento.
          </Surface>
        ) : (
                            ...
                            <div className="space-y-2">
                              ...
                              <dl className="flex flex-wrap gap-x-6 gap-y-1.5 font-sans text-sm text-(--color-text-secondary)">
                                <div className="flex items-center gap-2">
                                  ...
                                  <dd>{formatFull(session.scheduled_for)}</dd>
                                </div>
                                <div className="flex items-center gap-2">
                                  <Clock
                                    className="size-4 text-(--color-text-muted)"
                  ...
                  <li key={session.id}>
                    ...
                    >
                      ...
                      <p className="font-sans text-xs text-(--color-text-muted)">
                        {DAY_FMT.format(new Date(session.scheduled_for)).replace(
                          ',',
                          ' ·',
                        )}
                      </p>
